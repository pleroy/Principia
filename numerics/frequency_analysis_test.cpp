...
//TODO(phl): Tighten the tolerances.
TEST_F(FrequencyAnalysisTest, PoissonSeriesScalarProjection) {
  ...
  for (int i = 0; i <= 100; ++i) {
    EXPECT_THAT(projection4(t0_ + i * Radian / ω),
                AlmostEquals(series(t0_ + i * Radian / ω), 0, 3840));
  }
          ...
          ω_index == 0
              ...
              : ω_index == 1
                    ...
                    : ω_index == 2
                          ...
                          : AllOf(Gt(2.1e-17 * Metre), Lt(1.5e-3 * Metre)))
          << ω_index;
    }
    if (ω_index == ωs.size()) {
      return std::nullopt;
