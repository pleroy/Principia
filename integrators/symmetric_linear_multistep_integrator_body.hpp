...
Solve(Instant const& t_final,
  ...
  while (h <= (t_final - t.value) - t.error) {
    ...
    for (int d = 0; d < dimension; ++d) {
      ...
      current_step.displacements.push_back(current_position.ToDifference());
